...
//     // Keep the name field focused while typing
...
//                 className="focus-ring w-full cursor-pointer rounded-xl border border-white/10 bg-[#17232b] px-4 py-3 text-sm text-ink outline-none"
//               >
//                 {specialties.map((specialty) => (
//                   <option
//                     key={specialty}
//                     value={specialty}
//                     className="bg-[#17232b] text-white"
//                   >
//                     {specialty}
//                   </option>
//                 ))}
...
export default function AddDoctor() {
  ...
  const handleSubmit = async (event) => {
    ...
    const experienceYearsValue =
      formData.get('experienceYears')?.toString() || ''
    const ratingValue = formData.get('rating')?.toString() || ''
    const patientCountValue =
      formData.get('patientCount')?.toString() || ''
    ...
    try {
      ...
      console.error('Error adding doctor:', err)
      setError(
        err.message || 'Unable to add doctor.'
      )
    } finally {
      setLoading(false)
    }
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                className="focus-ring w-full cursor-pointer rounded-xl border border-white/10 bg-[#17232b] px-4 py-3 text-sm text-white outline-none"
              >
                {specialties.map((specialty) => (
                  <option
                    key={specialty}
